...
def Tower(len_1, selected_rod, destination_rod, central_rod):
    if len_1 == 0:
        return
    Tower(len_1 - 1, selected_rod, central_rod, destination_rod)
    print("shifted disk", len_1, "selected the rod", selected_rod, "destination of rod", destination_rod)
    Tower(len_1 - 1, central_rod, destination_rod, selected_rod)
...
print("PASCAL TRIANGLE USING RECURSIONS")


def pascal_triangle(len_1, original_length=n):
    if len_1 == 0:
        return
    pascal_triangle(len_1 - 1, original_length)
    print(' ' * (original_length - len_1), end=' ')  # for print the spaces(" ")
    begin = 1  # first number is always 1
    for i in range(1, len_1 + 1):
        print(begin, end=' ')

        begin = begin * (len_1 - i) // i  # using Binomial Coefficient
    print()
...
# creating object
student_1 = Student("Yuvraj", 21103009)
